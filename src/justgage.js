...
export const JustGage = function (config) {
  var obj = this
  ...
  obj.config = {
    ...
    // value : float
    // value gauge is showing
    pointerValue: kvLookup('pointerValue', config, dataset, 0, 'float'),
  ...
  obj.canvas.customAttributes.ndl = function (
    ...
    if (donut) {
      ...
      const { config: { pointerValue = 0 } = {} } = obj
      alpha =
        (1 - (pointerValue ? pointerValue : value - min) / (max - min)) *
        Math.PI
      Ro = w / 2 - w / 10
      Ri = Ro - (w / 6.666666666666667) * gws
